...
for i in np.arange(0, 7):
	var_ninio_WPV = np.mean(VAR.isel(month=i, realiz=index_ninio_WPV,
                                      drop=True), axis=0).to_array().squeeze()
	var_ninia_WPV = np.mean(VAR.isel(month=i, realiz=index_ninia_WPV, 
                                     drop=True), axis=0).to_array().squeeze()
	var_ninio_SPV = np.mean(VAR.isel(month=i, realiz=index_ninio_SPV, 
                                     drop=True), axis=0).to_array().squeeze()
	var_ninia_SPV = np.mean(VAR.isel(month=i, realiz=index_ninia_SPV, 
                                     drop=True), axis=0).to_array().squeeze()
	...
	var_ninia_all = np.mean(VAR.isel(month=i, realiz=index_ninia_all.values,
                                drop=True), axis=0).to_array().squeeze()
	tit = 'Composites S4 850hPa-Winds Conditioned - SPoV - ' + month[i]
	filename = FIG_PATH + '850winds' + '_composites_ENSO_' + month[i] +'_SPoV.png'
	plots.Plot850WindsCompositesENSOPoV(var_ninio_all - var_all,
                                     var_ninia_all- var_all, var_ninio_WPV - var_all,
                                     var_ninia_WPV - var_all, var_ninio_SPV - var_all,
                                     var_ninia_SPV - var_all, VAR.u.latitude,
                                     VAR.u.longitude, tit, filename)
